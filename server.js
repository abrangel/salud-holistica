...
const frontendUrl = process.env.FRONTEND_URL || 'https://salud-holistica.onrender.com';
app.use(cors({
  origin: frontendUrl
}));
...
// Ruta para la página de inicio (resuelve el error "Cannot GET /")
app.get('/', (req, res) => {
  res.status(200).send('Salud Holistica Backend API is running!');
});
...
app.post('/api/search', async (req, res) => {
  ...
  if (image && query) {
    promptText += `Identifica el objeto en la imagen y proporciona información sobre él, considerando el contexto de "${query}".\n`;
  } else if (image) {
  ...
  promptText += `
    {
      ...
      "dosis_recomendada": {
        "edad": {"es": "<dosis por edad en español>", "en": "<dosis por edad en inglés>"},
        "patologia": {"es": "<dosis por patología en español>", "en": "<dosis por pathology in English>"},
        "deporte": {"es": "<dosis por deporte en español>", "en": "<dosis por deporte en inglés>"}
      },
